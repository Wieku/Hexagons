 ...
 * @author stbachmann
 * @author Nathan Sweet */
public class ObjRender extends ShapeRenderer implements Disposable {
	...
	public ObjRender (int maxVertices) {
		this(maxVertices, null);
	}

	public ObjRender (int maxVertices, ShaderProgram defaultShader) {
	...
	public void setHeight(float y) {hejt=y;};
	...
	public void triangle (float x1, float y1, float x2, float y2, float x3, float y3) {
		...
		if (shapeType == ShapeType.Line) {
			renderer.color(colorBits);
			renderer.vertex(x1, hejt, y1);
			renderer.color(colorBits);
			renderer.vertex(x2, hejt, y2);

			renderer.color(colorBits);
			renderer.vertex(x2, hejt, y2);
			renderer.color(colorBits);
			renderer.vertex(x3, hejt, y3);
	...
	public void rectLine (float x1, float y1, float x2, float y2, float width) {
		...
		if (shapeType == ShapeType.Line) {
			renderer.color(colorBits);
			renderer.vertex(x1 + tx, hejt, y1 + ty);
			renderer.color(colorBits);
			renderer.vertex(x1 - tx, hejt, y1 - ty);

			renderer.color(colorBits);
			renderer.vertex(x2 + tx, hejt, y2 + ty);
			renderer.color(colorBits);
			renderer.vertex(x2 - tx, hejt, y2 - ty);
	...
	public void circle (float x, float y, float radius, int segments) {
		...
		if (shapeType == ShapeType.Line) {
			...
			for (int i = 0; i < segments; i++) {
				renderer.color(colorBits);
				renderer.vertex(x + cx, hejt, y + cy);
				float temp = cx;
				cx = cos * cx - sin * cy;
				cy = sin * temp + cos * cy;
				renderer.color(colorBits);

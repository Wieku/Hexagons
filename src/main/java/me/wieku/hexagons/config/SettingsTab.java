...
public class SettingsTab extends Table{
	...
	private SettingsTab(){
		...
		field.setTextFieldFilter(new TextFieldFilter(){
			...
			public boolean acceptChar(TextField arg0, char arg1) {
				...
				error.start(Main.getInstance().getAnimationManager());
	...
	public void show(){
		...
		showingTween.start(Main.getInstance().getAnimationManager());
	}
	...
	public void hide(){
		...
		hidingTween.start(Main.getInstance().getAnimationManager());
	}
	...
	public void build(String phrase){
		...
		for(Entry<String, HashMap<String, ArrayList<ConfigElement<?,?>>>> sec : ConfigEngine.searchMap(phrase).entrySet()){
			...
			tab1.add(new Label(sec.getKey().toUpperCase(), GUIHelper.getLabelStyle(Color.WHITE, 24))).right().expandX().row();
			...
			for(Entry<String, ArrayList<ConfigElement<?,?>>> subsec : sec.getValue().entrySet()){
				...
				tab2.add(new Label(subsec.getKey().toUpperCase(), GUIHelper.getLabelStyle(Color.WHITE, 20))).center().expandX();
				
				Node subNode = new Node(tab2);

				for(ConfigElement<?,?> el : subsec.getValue()){
					...
					tab.add(el.getActor()).right().padRight(10).expandX().uniform();
